...
public class EgovSampleController {
	...
	/**
	 * 한줄의견을 등록한다.
	 * @param comCommentVO - 조회 조건, 등록 정보가 담긴 VO (ModelAttribute명은 SessionAttributes명과 일치)
	 * @param bindingResult
	 * @param model
	 * @param status
	 * @return View
	...
	@ResponseBody
	public ResponseEntity<String> ajaxInsertLibFreeNoticeComment(@ModelAttribute("sampleVO") ComCommentVO comCommentVO, HttpServletRequest request) throws Exception
	{
	...
	@ResponseBody
	public ResponseEntity<String> ajaxLibFreeNoticeCommentList(@ModelAttribute("sampleVO") ComCommentVO comCommentVO, HttpServletRequest request) throws Exception
	{
		JSONObject jsonObject = new JSONObject();
		try {
			// 코맨트 등록
			// 페이지 정보 설정
			PaginationInfo paginationInfo = PaginationUtil.getPaginationInfo(comCommentVO);
			comCommentVO.setRecordCountPerPage(9999999);
			List<ComCommentVO> commentList = sampleService.retrieveComCommentList(comCommentVO);
			
			List<ComCommentVO> commentListTemp = new ArrayList<ComCommentVO>();
			for( ComCommentVO comCommentVO2: commentList) {
				comCommentVO2.setCommentContent(StringUtil.disableScript(comCommentVO2.getCommentContent()));
	...
	@ResponseBody
	public ResponseEntity<String> ajaxDeleteLibFreeNoticeComment(@ModelAttribute("sampleVO") ComCommentVO comCommentVO, HttpServletRequest request) throws Exception
	{
		JSONObject jsonObject = new JSONObject();	
		try {
			//삭제
			sampleService.delteLibFreeNoticeComment(comCommentVO);
			jsonObject.put("resultList", "삭재되었습니다.");
