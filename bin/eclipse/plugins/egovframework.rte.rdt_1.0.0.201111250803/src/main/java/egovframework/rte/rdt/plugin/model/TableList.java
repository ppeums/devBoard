...
public class TableList {
	...
	public TableList(RdtEditor instance) {
		...
		if (!store.getBoolean(RdtPreferencePage.DEFAULT_PROPERTY)) {
			...
			URL masterUrl = FileLocator.find(
					Activator.getDefault().getBundle(), new Path(
							MASTER_FILENAME), null);
			URL servicesUrl = FileLocator.find(Activator.getDefault()
					.getBundle(), new Path(SERVICES_FILENAME), null);
			try {
	...
	public boolean isInstalled(Service service) {
		...
		for (String s : service.getDependency()) { // s는 각서비스 마다 필요한
													// dependency들의 Id
			...
			for (Object o : InstallList) {
				...
				for (String s : service.getDependency()) { // s는 각서비스 마다 필요한
					...
					if (!isInstalled(s)) {
						...
						if (insDMap.get(s).getVersion()
								.compareTo(allDpendencyMap.get(s).getVersion()) < 0) { // 버전이
																						// 낮을때
							// 버전수정
							pom.changeVersion(insDMap.get(s).getId(),
									allDpendencyMap.get(s).getVersion());
						}
					}
				}// end for
			}
