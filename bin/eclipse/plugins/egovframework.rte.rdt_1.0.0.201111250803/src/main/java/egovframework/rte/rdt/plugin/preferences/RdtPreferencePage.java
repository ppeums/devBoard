 ...
 * @author 이영진
 */
public class RdtPreferencePage extends FieldEditorPreferencePage implements
	...
	public void createFieldEditors() {
		...
		mode = new BooleanFieldEditor(DEFAULT_PROPERTY,
				DEFAULT_TITLE, getFieldEditorParent());
		addField(mode);

		
		masterFilePath = new FileFieldEditor(
				MASTER_FILEPATH_PROPERTY, MASTER_FILEPATH_TITLE,
				getFieldEditorParent());
		masterFilePath.setFileExtensions(new String[] { "*.xml" });
		addField(masterFilePath);

		serviceFilePath = new FileFieldEditor(
				SERVICE_FILEPATH_PROPERTY, SERVICE_FILEPATH_TITLE,
				getFieldEditorParent());
		serviceFilePath.setFileExtensions(new String[] { "*.xml" });
		serviceFilePath.setChangeButtonText("B&rowse...");
		addField(serviceFilePath);

		DirectoryFieldEditor mavenPath = new DirectoryFieldEditor(MVN_PROPERTY,
				MVN_HOME_TITLE, getFieldEditorParent());
